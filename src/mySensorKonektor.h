 ...
 */
/*
replace MyGatewayTransportSerial.h pada library mysensors
*/
...
extern String mqttMsgInput;
extern bool mqttMsgReady;
...
bool gatewayTransportSend(MyMessage &message)
{
	...
	cekMySensors(String(protocolMyMessage2Serial(message)));
...
bool gatewayTransportAvailable(void)
{
	...
	{
		mqttMsgReady = false;
		for(uint8_t i= 0;i<mqttMsgInput.length();i++){
			_serialInputString[i] = mqttMsgInput.charAt(i);
		}
